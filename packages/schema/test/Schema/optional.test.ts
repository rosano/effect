  ...
  describe(`optional > { nullable: true, as: "Option" }`, () => {
    it("decoding / encoding", async () => {
      ...
      await Util.expectDecodeUnknownFailure(
        ...
        `({ a?: NumberFromString | undefined | null } <-> { a: Option<number> })
└─ From side transformation failure
   └─ { a?: NumberFromString | undefined | null }
      └─ ["a"]
         └─ NumberFromString | undefined | null
            ├─ Union member
            │  └─ NumberFromString
            │     └─ Transformation process failure
            │        └─ Expected NumberFromString, actual "a"
  ...
  describe("optional > { nullable: true, default: () => A }", () => {
    it("decoding / encoding", async () => {
      ...
      await Util.expectDecodeUnknownFailure(
        schema,
        { a: "a" },
        `({ a?: NumberFromString | undefined | null } <-> { a: number })
└─ From side transformation failure
   └─ { a?: NumberFromString | undefined | null }
      └─ ["a"]
         └─ NumberFromString | undefined | null
